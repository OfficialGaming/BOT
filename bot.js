...
//SET INTERVAL------------------------------------------------------------------
    ...
    if(test == 1 && (key != 0)){
        ...
        NOTIFY_CHANNEL.sendMessage('hey! ```nobody has checked walls in 30 minutes```', {tts: true});
        NOTIFY_CHANNEL.sendMessage('hey! ```nobody has checked walls in 30 minutes```', {tts: true});
...
client.on ('message', message => {
  if (message.content === prefix + "weewoo east") {
    ...
    NOTIFY_CHANNEL.sendMessage('@everyone **BEING RAIDED FROM EAST SIDE**', {tts: true});
    NOTIFY_CHANNEL.sendMessage('@everyone **BEING RAIDED FROM EAST SIDE**', {tts: true});
	NOTIFY_CHANNEL.sendMessage('@everyone **HMMMMMM**', {tts: true});
	NOTIFY_CHANNEL.sendMessage('@everyone **BEING RAIDED FROM EAST SIDE**', {tts: true});
    NOTIFY_CHANNEL.sendMessage('@everyone **BEING RAIDED FROM EAST SIDE**', {tts: true});
    NOTIFY_CHANNEL.sendMessage('@everyone **BEING RAIDED FROM EAST SIDE**', {tts: true});
    NOTIFY_CHANNEL.sendMessage('@everyone **BEING RAIDED FROM EAST SIDE**', {tts: true});
...
//LOGIN TOKEN-------------------------------------------------------------------
client.login('MzU4NTAxNjMxNDc1NTgwOTI4.DLzJAw.XHoAHEiezzoUpFUxt75zHc-md1Y');
